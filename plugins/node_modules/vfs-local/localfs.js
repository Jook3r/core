...
////////////////////////////////////////////////////////////////////////////////
    ...
    function mkfile(path, options, realCallback) {
        ...
        if (options.stream && options.stream.readable === false) {
    ...
    function tmuxspawn(ignored, options, callback) {
        ...
        if (options.capturePane) {
            ...
            var child;
            try {
                child = childProcess.spawn(TMUX, args.map(String), { env: process.env });
            } catch (err) {
                return callback(err);
            }
            ...
            // Buffer output server-side and return as plain strings.
            // capture-pane finishes before the client can attach stream listeners
            // (race condition), so we avoid the stream proxy entirely.
            var buf = "", errBuf = "";
            child.stdout.on("data", function(d) { buf += d; });
            child.stderr.on("data", function(d) { errBuf += d; });
            child.on("error", function() {});
            child.on("close", function() {
                callback(null, { output: buf, outputErr: errBuf });
            });
        ...
        if (options.listSessions) {
            // Use a safe separator that cannot appear in session names or tmux values.
            // \x01 gets corrupted somewhere in the execFile/VFS chain on some systems.
            var SEP = "|||";

            args = ["-u2", "-L", tmuxName,
                "list-sessions", "-F",
                    "#S" + SEP + "#{window_width}" + SEP + "#{window_height}",
                ";", "list-clients", "-F",
                    "C9CLIENT" + SEP + "#{client_session}"
            ];
            ...
            return _execFile(TMUX, args, options, function(e, data) {
                ...
                (data || "").split("\n").forEach(function(str) {
                    if (!str) return;
                    if (str.indexOf("C9CLIENT" + SEP) === 0) {
                        var name = str.slice(("C9CLIENT" + SEP).length);
                ...
                sessions.forEach(function(s) {
                    s.clientCount = clientCounts[s.name] || 0;
                });
