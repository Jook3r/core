...
////////////////////////////////////////////////////////////////////////////////
    ...
    function tmuxspawn(ignored, options, callback) {
        ...
        if (options.capturePane) {
            ...
            // Buffer all output server-side first, then emit after client attaches listeners.
            // capture-pane finishes very quickly so data would be lost in the race otherwise.
            var buf = "", errBuf = "";
            ...
            child.on("close", function() {
                var proxy = new Stream();
                proxy.stdout = new Stream();
                proxy.stderr = new Stream();
                proxy.stdout.readable = proxy.stderr.readable = true;
                callback(null, { process: proxy });
                setImmediate(function() {
                    if (buf) proxy.stdout.emit("data", buf);
                    if (errBuf) proxy.stderr.emit("data", errBuf);
                    proxy.emit("close");
                });
            });
