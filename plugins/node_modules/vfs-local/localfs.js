...
////////////////////////////////////////////////////////////////////////////////
    ...
    function tmuxspawn(ignored, options, callback) {
        ...
        if (options.capturePane) {
            ...
            console.log("[capturePane] cmd:", TMUX, args.join(" "));
            var child;
            try {
                child = childProcess.spawn(TMUX, args.map(String), { env: process.env });
            } catch (err) {
                console.log("[capturePane] spawn error:", err);
                return callback(err);
            }
            ...
            var _buf = "", _err = "";
            child.stdout.on("data", function(d) { _buf += d; });
            child.stderr.on("data", function(d) { _err += d; });
            child.on("close", function(code) {
                console.log("[capturePane] exit=" + code + " stdout=" + _buf.length + " stderr=" + JSON.stringify(_err));
            });

            // node 0.10.x emits error events if the file does not exist
            child.on("error", function(err) {
                console.log("[capturePane] child error:", err);
                child.emit("exit", 127);
            });
